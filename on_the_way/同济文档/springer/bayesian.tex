...
%==========================
%介绍bayesian网络
%%bayesian网络是一个对相关变量进行概率关系建模的图模型。当它结合统计手段，图模型在数据分析上就有好些优势。第一，因为该模型的建立是针对所有变量，所以它能够处理缺失部分数据的情况。第二，bayesian网络常被用来学习因果关系，因此能够用来获取对一个问题的理解以及结果的推理。第三，因为模型包容因果学和概率学，所以它是一个对先验知识和数据的理想的载体。第四，bayesian统计方法和bayesian网络的结合提供了一种有效的方法来避免过拟合数据的影响。
